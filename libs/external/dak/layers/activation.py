...
from ..utils.operators.chol_inv import mk_chol_inv
...
class InducedPriorUnit(nn.Module):
    def __init__(
        ...
        design_points = dyadic_design(
            deg=induced_level, input_lb=grid_bounds[0], input_ub=grid_bounds[1]
        ).points
        ...
        L_inv_T = (
            gpytorch.root_inv_decomposition(covar, method="cholesky")
            .to_dense()
            .clone()
            .detach()
        )
...
class Amk1d(nn.Module):
    def __init__(
        ...
        dyadic_design = design_class(dyadic_sort=True, return_neighbors=True)(
            deg=n_level, input_lb=input_lb, input_ub=input_ub
        )
        chol_inv = mk_chol_inv(
            dyadic_design=dyadic_design, markov_kernel=kernel, upper=True
        )  # [m, m] size tensor
        design_points = dyadic_design.points.reshape(-1, 1)  # [m, 1] size tensor

        self.register_buffer(
            "design_points", design_points
        )  # [m,d] size tensor, sparse grid points X^{SG} of dyadic sort
        self.register_buffer(
            "chol_inv", chol_inv
        )  # [m,m] size tensor, inverse of Cholesky decomposition of kernel(X^{SG},X^{SG})
...
class AMK(nn.Module):
    ...
    def forward(self, x):
        ...
        out = torch.flatten(x, start_dim=1).unsqueeze(
            dim=-1
        )  # reshape x of size [N, C, L] --> size [N, C*L, 1]
