...
from ..layers.functional import ScaleToBounds
...
class DAK(nn.Module):
    def __init__(
        ...
        self.gp_forward = LinearReparameterization(
            in_features=[
                self.gp_activation.in_features,
                self.gp_activation.num_inducing,
            ],
    ...
    def forward(
        self, x, num_mc=1, return_kl=True, return_sampling=True, add_noise=False
    ):
        """
        ...
        features = self.gp_activation(
            features
        )  # [N, P, M] size tensor --> [N, P*M] size tensor

        if return_sampling:
            output_ = []
            kl_ = []
            for mc_run in range(num_mc):
                output, kl = self.gp_forward(
                    features, return_kl=return_kl, return_sampling=True
                )
            ...
            res, kl = self.gp_forward(
                features, return_kl=return_kl, return_sampling=False
            )
...
class DAKMC(nn.Module):
    ...
    def __init__(
        ...
        self.embedding = nn.Linear(
            feature_extractor.fc.in_features, num_features, bias=False
        )
        ...
        self.gp_activation = InducedPriorUnit(
            ...
            kernel=LaplaceProductKernel(
                lengthscale=1.0
            ),  # Choose the general kernel you want to use
    ...
    @overload
    def forward(
        self, x, num_mc, return_kl: Literal[True]
    ) -> tuple[torch.Tensor, torch.Tensor]: ...
    @overload
    def forward(self, x, num_mc, return_kl: Literal[False]) -> torch.Tensor: ...
...
class MultitaskDAK(nn.Module):
    ...
    def _init_params(self):
        self.gp_activation.kernel.lengthscale = 1.0
