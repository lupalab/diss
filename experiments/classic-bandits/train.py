...
def eval_env(
    ...
    while env.has_next():
        ...
        if to_log_additioanl_metrics:
            # TODO bacts seems off with inidices
...
def make_init_queries(
    ...
    reward_est: disslib.RewardEst,
    smrt_fitter: Optional[disslib.SmartFitter],
    init_capital: int,
    plf: pl.Fabric,
    ...
    reward_est.set_train_data_(init_inputs, init_targets, init_infos)
    # fit score estimator
    init_metrics = (
        reward_est.fit_(plf) if smrt_fitter is None else smrt_fitter.smart_fit_(plf)
    )
    init_metrics.update({"n_obs": len(reward_est.train_inputs)})
    return init_metrics
...
def run_trial(
    ...
    _reward_est = (
        reward_est
        if smrt_fitter is None or not smrt_fitter.is_compatible(strat)
        else smrt_fitter
    )
    # determine next batch of queries
    bctxs, bacts, bimps, bctxs_infos = strat.suggest_next_queries(_reward_est, plf)
    bctxs = bctxs.to(device=plf.device)
    bacts = bacts.to(device=plf.device)
    bimps = bimps.to(device=plf.device)
    bctxs_infos = bctxs_infos.to(device=plf.device)
    # evaluate suggested ctx-act pair
    # bxacts: th.Tensor = strat.env.decompose_acts(bacts)["fcinds"]
    binputs: th.Tensor = th.cat((bctxs, bacts), dim=1).to(device=plf.device)
    btargets, binfos = strat.env.compute_rewards(bctxs, bacts, bctxs_infos)
    btargets = btargets.to(device=plf.device)
    # compute actual improvement
    bpms: th.Tensor = _reward_est.get_posterior_mean(_reward_est(binputs))
    brimps: th.Tensor = btargets - bpms
    brimps = brimps.to(device="cpu")
    # add queries to estimator
    reward_est.add_to_train_data_(binputs, btargets, binfos)
    metrics_d: dict[str, float] = {
        ...
        "n_obs": len(reward_est.train_inputs),
    }
    if not (strat.support_lazy_fit and reward_est._enable_lazy_fit):
        fit_metrics_d: dict[str, float] = (
            _reward_est.smart_fit_(plf)
            if isinstance(_reward_est, disslib.SmartFitter)
            else _reward_est.fit_(plf)
        )
...
def maximize(
    ...
    # initialize structure estimator
    reward_est.to(plf.device)
    # run trials
    pbar = tqdm.trange(n_iter, desc="maximize", dynamic_ncols=True, leave=False)
    for itr in pbar:
        try:
            metrics_d: dict[str, float]
            if itr == 0:
                metrics_d = make_init_queries(
                    ...
                    smrt_fitter=smrt_fitter,
                    init_capital=init_capital,
                    plf=plf,
                    rseed=init_capital_rseed,
                )
            else:
                metrics_d = run_trial(
                    reward_est=reward_est, smrt_fitter=smrt_fitter, strat=strat, plf=plf
                )
            if (
                (smrt_fitter is not None and smrt_fitter.is_compatible(strat))
                or (strat.support_lazy_fit and reward_est._enable_lazy_fit)
            ) and (((itr + 1) == n_iter or itr % eval_every_n_iter == 0)):
                metrics_d.update(reward_est.fit_(plf))
            ...
            if ckpt_p is not None and (
                ...
                plf.save(
                    os.path.join(ckpt_p, f"itr_{itr}.ckpt"), reward_est.state_dict()
                )
    ...
    if ckpt_p is not None:
        plf.save(os.path.join(ckpt_p, "itr_end.ckpt"), reward_est.state_dict())
...
def make_smrt_fitter(
    strat: disslib.OptStrat, reward_est: disslib.RewardEst
) -> disslib.SmartFitter | None:

    if isinstance(reward_est, disslib.estimators.XGBoostRewardEst):
        if isinstance(
            strat,
            (
                disslib.strategies.RandomOptStrat,
                disslib.strategies.MTSPM,
                disslib.strategies.TS,
            ),
        ):
            return disslib.smart_fitters.XGBoostRewardEstFitOnlyOne(reward_est)
    elif isinstance(reward_est, disslib.estimators.StructureRewardEstBase):
        if isinstance(
            strat,
            (
                disslib.strategies.RandomOptStrat,
            ...
            return disslib.smart_fitters.StructureRewardEstFitOnlyOne(reward_est)
    return None
...
def main(cfg: MainConf, logger: logging.Logger):
    ...
    strat: disslib.OptStrat = hd.utils.instantiate(cfg.strat, env=tenv)
    ...
    maximize(
        train_env=tenv,
        reward_est=reward_est,
        smrt_fitter=make_smrt_fitter(strat, reward_est),
        strat=strat,
