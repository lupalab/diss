...
# ============================================================================
# VLLM Environment Code for Bar Crawl Dataset
# ============================================================================
def make_vllm_diss_components(
    ...
    bdm = BarCrawlVLLMSubsetFeatureClassifier(
        n_neighs=n_neighs,
        tdata=bdmtdata,
...
# ============================================================================
# Class Interface (Bar Crawl VLLMSubsetFeatureClassifier)
# ============================================================================
class BarCrawlVLLMSubsetFeatureClassifier(mymodels.classifiers.SubsetFeatureClassifier):
    """Bar Crawl Heavy Drinking VLLMSubsetFeatureClassifier (Server Mode).

    Connects to a running vLLM server via OpenAI-compatible API.
    Start the server first: python scripts/vllm_server.py

    This classifier predicts heavy drinking (TAC >= 0.08 g/dl) based on
    accelerometer features extracted from smartphone sensor data during
    a bar crawl study (Killian et al., 2019).
    ...
    Example:
        ...
        >>> classifier = BarCrawlVLLMSubsetFeatureClassifier(
        ...     n_neighs=3,
        ...     tdata=tdata,
    ...
    def forward(
        ...
        """Forward pass - query LLM for intoxication risk predictions.
    ...
    def query(
        ...
        """Query vLLM with batch of accelerometer samples.

        Args:
            xs: Accelerometer features (B, F)
        ...
        if save_prompts_to:
            ...
            prompts_file = os.path.join(save_prompts_to, "bar-crawl-prompts.json")
        ...
        if save_prompts_to:
            os.makedirs(save_prompts_to, exist_ok=True)
            responses_file = os.path.join(save_prompts_to, "bar-crawl-responses.txt")
...
# ============================================================================
# Constants (Bar Crawl Dataset - ALL Features)
# ============================================================================
...
_AXES: Final[list[str]] = ["x", "y", "z"]
_TIME_DOMAIN_SUFFIXES: Final[list[str]] = [
    "mean",
    "std",
    "median",
    "zcr",
...
_FREQ_DOMAIN_SUFFIXES: Final[list[str]] = [
    "spectral_entropy",
    "spectral_centroid",
    "spectral_spread",
    "spectral_rolloff",
...
FEATURE_DESCRIPTIONS: Final[dict[str, str]] = {
    ...
    "median": "median value",
    "zcr": "zero crossing rate (signal direction changes)",
    "max": "maximum value",
    "min": "minimum value",
    ...
    "spectral_spread": "spread of spectrum around centroid",
    "spectral_rolloff": "frequency below which 90% energy is contained",
    "max_frequency": "frequency with highest power",
    "average_power": "average spectral power",
    "spectral_peak_ratio": "ratio of largest to second-largest peak",
    # Categorical
    "phonetype": "phone type (Android=0, iPhone=1)",
}
...
def inverse_transform_xs(
    ...
    if stdsclr is not None:
        ...
        xs_orig = stdsclr.inverse_transform(xs_temp)
        # Restore NaN positions
        xs_orig = np.where(nan_mask, np.nan, xs_orig)
    else:
        xs_orig = xs_numpy
    # Convert to DataFrame
    xs_df = pd.DataFrame(xs_orig, columns=FEATURE_NAMES)
    return xs_df
...
def dataframe_to_json(df: pd.DataFrame, include_outcome: bool = False) -> dict:
    ...
    Args:
        ...
        include_outcome: Whether to include intoxication outcome

    Returns:
        JSON dict with accelerometer data
    """
    if len(df) == 1:
        # Single sample
        row = df.iloc[0]
        ...
        # Group features by axis for better readability
        for axis in _AXES:
            axis_features = {}
            for suffix in _TIME_DOMAIN_SUFFIXES + _FREQ_DOMAIN_SUFFIXES:
                feat = f"{axis}_{suffix}"
                if feat in df.columns and pd.notna(row[feat]):
                    axis_features[suffix] = round(float(row[feat]), 4)
            if axis_features:
                sample_json[f"{axis}_axis"] = axis_features

        # Add phonetype
        if "phonetype" in df.columns and pd.notna(row["phonetype"]):
            sample_json["phonetype"] = (
                "iPhone" if int(row["phonetype"]) == 1 else "Android"
            )

        # Add outcome if requested
        if include_outcome and "intoxication_outcome" in df.columns:
            sample_json["intoxication_outcome"] = int(row["intoxication_outcome"])

        return {"sample": sample_json}

    else:
        # Multiple samples (k-NN cases)
        cases = []
        for _, row in df.iterrows():
            case_dict = {}

            # Group features by axis
            for axis in _AXES:
                axis_features = {}
                for suffix in _TIME_DOMAIN_SUFFIXES + _FREQ_DOMAIN_SUFFIXES:
                    feat = f"{axis}_{suffix}"
                    if feat in df.columns and pd.notna(row[feat]):
                        axis_features[suffix] = round(float(row[feat]), 4)
                if axis_features:
                    case_dict[f"{axis}_axis"] = axis_features
            ...
            if include_outcome and "intoxication_outcome" in df.columns:
                case_dict["intoxication_outcome"] = int(row["intoxication_outcome"])
...
def _make_prompt(
    ...
    """Generate prompt for single accelerometer sample.
    ...
    if n_neighs > 0:
        ...
        knnctxs_df["intoxication_outcome"] = (
    ...
    # Convert current sample to JSON
    ctxs_df = inverse_transform_xs(ctxs, stdsclr, feature_preprocessor).dropna(axis=1)
    sample_json = dataframe_to_json(ctxs_df, include_outcome=False)

    # Build system message
    system_msg = """You are an AI assistant specializing in analyzing smartphone accelerometer data to assess intoxication levels.

You are analyzing accelerometer sensor data from a smartphone carried by a participant during a bar crawl study.
The goal is to predict whether the participant is intoxicated (TAC >= 0.08 g/dl, the legal limit).

Feature descriptions:
- Time-domain features (per axis x, y, z): mean, std, median, zcr (zero crossing rate), max, min, max_abs, min_abs, skewness, kurtosis, rms
- Frequency-domain features (per axis x, y, z): spectral_entropy, spectral_centroid, spectral_spread, spectral_rolloff, max_frequency, average_power, spectral_peak_ratio
- phonetype: Android (0) or iPhone (1)

Intoxication indicators in accelerometer data:
- Higher variability (std, spectral_spread) suggests unstable movement
- Higher spectral entropy suggests irregular/chaotic movement patterns
- Higher zero crossing rate may indicate tremors or erratic motion
- Lower spectral centroid may indicate slower, more deliberate movements

Output ONLY valid JSON in this exact format:
{
  "values": {
    "intoxication_risk": <0-4>,
    "confidence": <0-4>
  }
}

Risk scale: 0=sober (very low risk), 1=likely sober, 2=uncertain, 3=likely intoxicated, 4=intoxicated (very high risk)
Confidence scale: 0=very unsure, 1=unsure, 2=moderate, 3=confident, 4=very confident"""

    # Build user message
    if knn_json:
        user_msg = f"""Accelerometer Data (10-second window):
{json.dumps(sample_json, indent=2)}

Similar Cases (k-NN, intoxication_outcome: 0=sober, 1=intoxicated):
{json.dumps(knn_json, indent=2)}

Assess intoxication risk based on the accelerometer patterns and similar case outcomes.
Provide assessment in the JSON format specified."""
    else:
        user_msg = f"""Accelerometer Data (10-second window):
{json.dumps(sample_json, indent=2)}

Assess intoxication risk based on the accelerometer patterns.
Provide assessment in the JSON format specified."""
...
if __name__ == "__main__":
    ...
    def test_bar_crawl_predict_proba(
        n_samples: int = 50,
        n_neighs: int = 3,
        server_url: str = "http://localhost:8000/v1",
    ) -> dict[str, float]:
        """Test BarCrawlVLLMSubsetFeatureClassifier using predict_proba output.
        ...
        print(f"Initializing BarCrawlVLLMSubsetFeatureClassifier...")
        ...
        print("\n" + "=" * 60)
        print("Bar Crawl Predict Proba Test Results")
        print("=" * 60)
    ...
    def test_bar_crawl_query_json_errors(
        n_samples: int = 10,
        n_neighs: int = 0,
        server_url: str = "http://localhost:8000/v1",
    ) -> dict[str, float]:
    ...
    test_bar_crawl_query_json_errors(
        n_samples=10,
        n_neighs=0,
