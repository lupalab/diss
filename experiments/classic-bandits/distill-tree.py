...
def main(cfg: MainConf, logger: logging.Logger):
    ...
    # cluster gathered actions
    # this inner function tries to release gpu vram occupied by kms
    def _compute_centroids() -> tuple[th.Tensor, th.Tensor, th.Tensor]:
        kms = skl_cluster.KMeans(n_clusters=cfg.n_clusters, random_state=279).fit(
            acts.numpy(force=True)
        )
        cs = th.as_tensor(kms.predict(acts.numpy(force=True)))
        cids: th.Tensor = th.unique(cs)
        centroids: th.Tensor = th.stack(
            [th.mean(acts[cs == _cid].to(dtype=th.float32), dim=0) for _cid in cids],
            dim=0,
        )
        centroids_b: th.Tensor = centroids > 0.5
        del kms
        return cs, centroids, centroids_b

    cs, centroids, centroids_b = _compute_centroids()
    gc.collect()
    th.cuda.empty_cache()
    # train decision tree
    dtc: skl_tree.DecisionTreeClassifier = hd.utils.instantiate(cfg.dtc)
    dtc.fit(outs["ctxs"].numpy(force=True), cs.numpy(force=True))
    th.save(
        {
            "dtc": dtc,
            "centroids": centroids,
    ...
    # tmetrics_d: dict[str, Any] = eval_env_with_dtc(
    #     env=tenv,
    #     dtc=dtc,
    #     centroids_b=centroids_b,
    #     plf=plf,
    ...
    if venv is not None:
        vmetrics_d: dict[str, Any] = eval_env_with_dtc(
            env=venv,
            dtc=dtc,
            centroids_b=centroids_b,
            plf=plf,
    ...
    if tstenv is not None:
        tstmetrics_d: dict[str, Any] = eval_env_with_dtc(
            env=tstenv,
            dtc=dtc,
            centroids_b=centroids_b,
            plf=plf,
